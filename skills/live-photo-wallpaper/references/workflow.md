...
For animated wallpaper, derive the cover from the decoded opening video frame whenever possible. A frame from the same shot may work after target-device testing, but a different pose, framing, or scene is not a supported wallpaper input. The two resources do not need to be exported from one original asset, but they must remain visually continuous at the transition.
...
When animation is unavailable, first make a fresh control package using the decoded opening video frame as the cover. If that enables animation, the original cover is not a supported wallpaper match; do not change metadata or encoding to compensate. Otherwise confirm 60 fps, HEVC Main, `hvc1`, `1/600`, and required metadata. Test every candidate with a fresh UUID on the device and alter one variable at a time.
